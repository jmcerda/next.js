...
///
/// Required for liveness in [`begin_exclusion`](SnapshotCoordinator::begin_exclusion): snapshot/GC
/// runs as a `tokio::spawn` background job, so it parks on a worker while waiting on operations
/// that themselves need workers to be polled. Without the hand-off a single-worker (or fully
/// contended) runtime deadlocks. Elsewhere (`begin_operation`, `suspend_point`) it is throughput
/// only: the pass runs regardless, but parking a worker starves whatever else could have used it.
///
...
impl<O> SnapshotCoordinator<O> {
    ...
    pub fn begin_operation(&self) -> OperationGuard<'_, O> {
        ...
        fn wait_for_exclusion_to_complete<O>(this: &SnapshotCoordinator<O>) {
            ...
            if state.snapshot_requested || state.gc_requested {
                ...
                // Throughput: hand the worker slot back while parked. See `block_off_runtime`.
                block_off_runtime(|| {
    ...
    fn begin_exclusion(
        ...
        if (active & !REQUEST_BITS) != 0 {
            ...
            // liveness. See `block_off_runtime`.
            block_off_runtime(|| {
...
impl<'a, O> GcPhase<'a, O> {
    /// Atomically transitions from the GC phase directly into a snapshot phase
...
mod tests {
    ...
    /// The exclusion kinds each shared-behaviour test below runs against. `begin_snapshot` and
    /// `begin_gc` both delegate to `begin_exclusion`, differing only in which request flag the
    /// closure sets — so these behaviours are covered once per kind here rather than in a pair of
    /// near-identical tests per behaviour. GC-specific behaviour (the `into_snapshot` handoff, the
    /// mutual-exclusion panics) is tested separately below.
    ///
    /// A trait rather than a table of fn pointers because the phase borrows the coordinator, and a
    /// `fn` pointer would force that borrow to `'static`.
    trait Exclusive {
        /// Human label used in assertion messages.
        const LABEL: &'static str;
        /// Whether this kind exposes the suspended-operation list (`GcPhase` does not).
        const REPORTS_SUSPENDED: bool;
        /// Guard whose `Drop` ends the exclusion.
        type Phase<'a>;

        /// Begins the exclusion, returning the guard and the number of operations that suspended to
        /// let it start (0 when the kind does not report them).
        fn begin(coord: &SnapshotCoordinator<Op>) -> (Self::Phase<'_>, usize);
    }

    struct SnapshotKind;
    impl Exclusive for SnapshotKind {
        const LABEL: &'static str = "snapshot";
        const REPORTS_SUSPENDED: bool = true;
        type Phase<'a> = SnapshotPhase<'a, Op>;

        fn begin(coord: &SnapshotCoordinator<Op>) -> (Self::Phase<'_>, usize) {
            let phase = coord.begin_snapshot();
            let suspended = phase.suspended_operations().len();
            (phase, suspended)
        }
    }

    struct GcKind;
    impl Exclusive for GcKind {
        const LABEL: &'static str = "gc";
        const REPORTS_SUSPENDED: bool = false;
        type Phase<'a> = GcPhase<'a, Op>;
    ...
    fn no_ops_proceeds_immediately<E: Exclusive>() {
        let coord = SnapshotCoordinator::<Op>::new();
        let (phase, suspended) = E::begin(&coord);
        assert!(
            coord.exclusion_pending(),
            "{}: exclusion should be pending",
            E::LABEL
        );
        assert_eq!(
            suspended,
            0,
            "{}: nothing was running, so nothing suspended",
            E::LABEL
        );
        drop(phase);
        assert!(
            !coord.exclusion_pending(),
            "{}: dropping the phase must clear the request",
            E::LABEL
        );
    }
    ...
    fn snapshot_with_no_ops_proceeds_immediately() {
        no_ops_proceeds_immediately::<SnapshotKind>();
    }

    #[test]
    fn gc_with_no_ops_proceeds_immediately() {
        no_ops_proceeds_immediately::<GcKind>();
    }

    fn waits_for_ops_to_drain<E: Exclusive>() {
        ...
        let thread = thread::spawn({
            ...
            move || {
                let _phase = E::begin(&coord2);
                started.store(1, Ordering::Release);
            }
        });

        // The pass can't proceed past `begin_*` while we hold `g`.
        wait_for_snapshot_pending(&coord);
        assert_eq!(
            started.load(Ordering::Acquire),
            0,
            "{}: must not start while an operation is in flight",
            E::LABEL
        );
        ...
        assert_eq!(
            started.load(Ordering::Acquire),
            1,
            "{}: must proceed once operations drain",
            E::LABEL
        );
    }
    ...
    fn snapshot_waits_for_ops_to_drain() {
        waits_for_ops_to_drain::<SnapshotKind>();
    }

    #[test]
    fn gc_waits_for_ops_to_drain() {
        waits_for_ops_to_drain::<GcKind>();
    }

    fn new_operation_blocks_during<E: Exclusive>() {
        let coord = Arc::new(SnapshotCoordinator::<Op>::new());
        let (phase, _) = E::begin(&coord);
        ...
        // Wait until the worker is alive and about to call begin_operation. We can't directly
        // observe it entering begin_operation (its fetch_add is transient — it backs out and parks
        // before we can sample), but since we hold `phase` the worker provably cannot set
        // started_op=1 from anywhere inside begin_operation. So observing started_op==0 after the
        // worker is running and on its way in is a real check, not a vacuous one.
    ...
    fn new_operation_blocks_during_snapshot() {
        new_operation_blocks_during::<SnapshotKind>();
    }

    #[test]
    fn new_operation_blocks_during_gc() {
        new_operation_blocks_during::<GcKind>();
    }

    /// `suspend_point` lets a waiting pass proceed, and (for a snapshot) the suspended operation is
    /// recorded so it can be replayed.
    fn suspend_point_lets_exclusion_proceed<E: Exclusive>() {
        let coord = Arc::new(SnapshotCoordinator::<Op>::new());
        let g = coord.begin_operation();
        ...
        let pass_thread = thread::spawn({
            let done = done.clone();
            move || {
                let (phase, suspended) = E::begin(&coord2);
                if E::REPORTS_SUSPENDED {
                    assert_eq!(
                        suspended,
                        1,
                ...
                // Hold the exclusion briefly so the suspend_point thread observes the request flag
                // still set after waking.
        ...
        wait_for_snapshot_pending(&coord);
        // The pass is waiting for our operation to drain; suspending should let it proceed. The
        // suspend closure must run for *both* kinds — see `suspend_point`'s comment on why GC
        // suspensions are recorded too (a GC phase can hand its exclusion straight to a snapshot).
        ...
        // `suspend_point` returns only once the pass has finished.
        assert_eq!(
            done.load(Ordering::Acquire),
            1,
            "{}: suspend_point must not return before the pass completes",
            E::LABEL
        );
        assert_eq!(
        ...
        pass_thread.join().unwrap();
        drop(g);
    }

    #[test]
    fn suspend_point_lets_snapshot_proceed() {
        suspend_point_lets_exclusion_proceed::<SnapshotKind>();
    }

    #[test]
    fn suspend_point_lets_gc_proceed() {
        suspend_point_lets_exclusion_proceed::<GcKind>();
    }

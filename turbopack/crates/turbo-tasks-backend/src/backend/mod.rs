...
use gc::{GC_ROOT_TTL, LastGcStats};
...
impl TurboTasksBackend {
    ...
    pub fn new(mut options: BackendOptions, backing_storage: TurboBackingStorage) -> Self {
        ...
        // Resolved once here rather than re-read per pass: the env var cannot change within a
        // process, and `BackendOptions::gc_root_ttl` is fixed for the backend's lifetime.
    ...
    fn recycle_freed_task_ids(&self, freed: Vec<TaskId>) {
        ...
        tracing::trace!(
            ...
            resident = self.storage.resident_persistent_task_count(),
        );
        for id in released {
            debug_assert!(
                !id.is_transient(),

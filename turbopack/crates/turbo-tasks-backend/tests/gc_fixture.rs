...
/// The diamond *reader* `A`: `B` arrives already resolved, so reading it records a forward
/// dependency without making `B` a child (only *calling* a task creates a child edge).
#[turbo_tasks::function]
pub async fn diamond_reader(target: ResolvedVc<u32>) -> Result<Vc<u32>> {
    Ok(Vc::cell(1 + *target.await?))
}

/// The diamond root: parents each `A`/`B` pair as siblings. `fanout` is an argument so each caller
/// gets distinct task instances (it is part of the cache key) and can widen the race window.
#[turbo_tasks::function]
...
/// [`diamond_root`] as an op, for tests that need an `OperationVc` to take a `task_id()` from and
/// pin as a durable root.
#[turbo_tasks::function(operation, root)]

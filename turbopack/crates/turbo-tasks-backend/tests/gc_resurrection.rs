...
/// Width of the reader/diamond subtrees. These tests are race tests — the wider the subtree, the
/// more chances for the interleaving under test — so this is set well above the minimum that
/// *describes* the shape. It is free to keep it high: this binary runs in ~0.2s either way, and
/// dropping to 128 measured no faster.
...
async fn gc_rebalances_aggregation_and_cascades_in_one_pass() {
    ...
    // No task from the collected subtree may have been recorded as a GC root: a leaf that reached
    // the post-drain scan still holding its dangling `upper` edge to the deleted `reader` would
    // land in the persisted roots map as `MostRecent`, which never ages out.
    //
    // The map is expected to be non-empty — the three `(operation, root)` tasks read at the top
    // level of the `run_once` are parentless and anchored by the transient `Once` task, which is
    // exactly what a GC root is. The subtree is `FANOUT` leaves wide, far more than this bound of
    // 3, so a leaked leaf could not hide under it.
    //
    // If this ever fires it is a finding about the aggregation graph — an edge outliving the task
    // it points at — not a reason to narrow `gc_is_root`.
    let roots = tt2.backend().persisted_gc_roots_for_testing();
    assert!(
        roots.len() <= 3,
        "only the top-level operation roots should be tracked, got {} entries: {roots:?}",
        roots.len()
    );

...
describe('mcp-server get_errors tool', () => {
  ...
  it('should send runtime error state only to its originating HMR client', async () => {
    const first = collectRuntimeErrorStateMessages()
    const second = collectRuntimeErrorStateMessages()
    const [firstSession, secondSession] = await Promise.all([
      ...
      launchStandaloneSession(next.url, '/', second.beforePageLoad),
    ])
    ...
    try {
      ...
      expect(
        second.messages
          .flatMap((state) => state.errors)
          .find((error) => error.message === 'Test event runtime error')
      ).toBeUndefined()
    } finally {
